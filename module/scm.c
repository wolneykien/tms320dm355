			      ...
			      char *buf)
{
  ...
  state = 2 - read_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_SR_PD) \
    - read_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_LPMODEN);
			       ...
			       const char *buf, size_t count)
{
  if (count > 0) {
    ...
    if (strncmp(buf, "0", 1) == 0) {
      ...
      write_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_SR_PD, 1);
      write_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_LPMODEN, 1);
    } else if (strncmp(buf, "1", 1) == 0) {
      DBG("Entering DDR self-refresh mode\n");
      write_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_SR_PD, 0);
      write_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_LPMODEN, 1);
    } else {
      DBG("Entering normal DDR mode\n");
      write_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_SR_PD, 0);
      write_reg_part(DA8XX_CTL_BASE, SDRCR, SDRCR_LPMODEN, 0);
    }
  }
